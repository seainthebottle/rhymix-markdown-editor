...
class RhymixMarkdownEditor {
    ...
    build(content_key) {
        ...
        let editor_id = this.id;
        let rmde_btn_preview = editor_id + " .rmde_btn_preview";
        let rmde_editor_ruler_for_scroll = editor_id + " #rmde_editor_ruler_for_scroll";
        let rmde_editor_textarea = editor_id + " #rmde_editor_textarea";
        let rmde_preview = editor_id + " .rmde_preview";
        let rmde_status_mathjax_on = editor_id + " #rmde_status_mathjax_on";
        ...
        $(editor_id).html(html_data);
        // 초기에 preview는 숨긴다.
        $(rmde_preview).hide();
        // MathJax가 로딩되어 있으면 표시한다.
        if (typeof MathJax !== "undefined")
            $(rmde_status_mathjax_on).css("display", "inline-block");
        else
            $(rmde_status_mathjax_on).css("display", "none");
    ...
    togglePreview() {
        let editor_id = this.id;
        let rmde_root = editor_id + " .rmde_class_root";
        let rmde_toolbar = editor_id + " .rmde_toolbar";
        let rmde_editor = editor_id + " .rmde_editor";
        let rmde_editor_textarea = editor_id + " #rmde_editor_textarea";
        let rmde_preview = editor_id + " .rmde_preview";
        let rmde_preview_title = editor_id + " .rmde_preview_title";
        let rmde_preview_main = editor_id + " .rmde_preview_main";
    ...
    movePreviewPosition(
        ...
        if (linenum == 0)
            $(rmde_preview_main).stop(true).animate({ scrollTop: 0, }, 100, "linear");
        else
